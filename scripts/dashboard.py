"""
dashboard.py — Research-backed real-time AlgoTrading dashboard.

Layout  : Sidebar + 3 main tabs (Market Overview | Stock Detail | TFT Predictions)
Data    : Redis STOCK:* hashes (viz_consumer) + PRED:*:quantiles (inference_consumer)
Design  : Hybrid table+heatmap primary view, candlestick drill-down, session-state sparklines.
          Based on research of Bloomberg, TradingView, Zerodha Kite, Finviz UX patterns.
...
Prerequisites:
    ...
    - viz_consumer    : py -m src.consumers.viz_consumer
    - Kafka producer  : py scripts/kite_to_kafka.py  (or kafka_live_view.py for mock)
"""
...
MAX_HISTORY = 200   # LTP snapshots per stock (200 × 2s ≈ 6.7 min of session history)
...
st.markdown(
    "<style>[data-testid='stMetricValue']{font-size:1.25rem}</style>",
    unsafe_allow_html=True,
)

# ── Session state (initialized once per browser session) ──────────────────────
st.session_state.setdefault("selected_stock", None)   # str | None
st.session_state.setdefault("price_history",  {})     # {symbol: [ltp, ...]}
st.session_state.setdefault("refresh_count",  0)      # int
...
def fetch_predictions(r: redis.Redis, symbols: list) -> pd.DataFrame:
    ...
    pipe = r.pipeline()
    for sym in symbols:
        pipe.hgetall(f"PRED:{sym}:quantiles")
    results = pipe.execute()
    rows = []
...
def market_status() -> tuple[str, str]:
    ...
    wd = now.weekday() < 5
    if wd and (9 * 60 + 15) <= t <= (15 * 60 + 30):
        return "open",   "🟢 Market Open"
    if wd and (9 * 60) <= t < (9 * 60 + 15):
...
def _build_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    chg_vals   = df["Chg%"].tolist()
    ltps       = df["LTP"].tolist()
    sectors    = [SECTOR_MAP.get(s, "Other") for s in syms]
    unique_sec = sorted(set(sectors))
    n          = len(unique_sec)

    labels   = syms + unique_sec
    parents  = sectors + [""] * n
    values   = volumes + [0] * n
    colors   = chg_vals + [0.0] * n
    custom   = [[ltps[i], chg_vals[i], volumes[i]] for i in range(len(syms))]
    custom  += [[0, 0, 0]] * n
    ...
    fig = go.Figure(go.Treemap(
        ...
        hovertemplate=(
            "<b>%{label}</b><br>"
            "LTP: ₹%{customdata[0]:.2f}<br>"
            "Chg: %{customdata[1]:+.2f}%<br>"
...
def _build_candlestick(row: pd.Series, symbol: str) -> go.Figure:
    ...
    for label, val, color, side in [
        ("Open", row["Open"], "#aaaaaa", "bottom left"),
    ...
    fig.update_layout(
        title=f"{symbol} — Today's Bar  (close = live LTP)",
        height=320,
...
def _build_sparkline(history: list, symbol: str) -> go.Figure | None:
    ...
    fig.update_layout(
        title=f"{symbol} — LTP this session ({len(history)} ticks)",
        height=170,
...
def _render_table_view(df: pd.DataFrame, filter_fn=None, key: str = "tbl"):
    ...
    try:
        ...
        styled = display   # jinja2 < 3.1.5: plain table fallback
    ...
    # Row click → update selected stock
    try:
...
# ── Sidebar shell (filled after data is fetched) ──────────────────────────────
with st.sidebar:
...
with h1:
    st.title("AlgoTrading Dashboard")
with h2:
    st.success("Redis ✓") if connected else st.error("Redis ✗")
with h3:
    st.caption(mkt_label)
with h4:
...
if not df.empty:
    for _, row in df.iterrows():
        ...
        if len(hist) > MAX_HISTORY:
            st.session_state["price_history"][sym] = hist[-MAX_HISTORY:]
...
if df.empty:
    st.info(
        "**No live stock data yet.**  \n"
        "Start the viz consumer:\n"
...
# ── Main tabs ─────────────────────────────────────────────────────────────────
tab_overview, tab_detail, tab_predictions = st.tabs([
...
with tab_overview:
    subtab_table, subtab_heat = st.tabs(["Table View", "Heatmap View"])
    ...
    with subtab_table:
        ...
        _render_table_view(df, filter_fn=filter_fns[view_mode], key=f"tbl_{view_mode}")

    with subtab_heat:
        st.caption(
            "Cell **size** = volume traded · Cell **color** = % change  "
            "(dark = ~0%, green = gain, red = loss) · Click a stock to select it."
        )
        fig_heat  = _build_heatmap(df)
        heat_evt  = st.plotly_chart(
            fig_heat,
            on_select="rerun",
            key="heatmap_treemap",
            use_container_width=True,
        ...
        try:
            ...
            if pts:
                lbl = pts[0].get("label", "")
                if lbl in symbols and lbl != st.session_state.get("selected_stock"):
...
with tab_detail:
    ...
    if not selected or selected not in symbols:
        st.info("Select a stock by clicking a row in the Table View or a cell in the Heatmap.")
    else:
        row     = df[df["Symbol"] == selected].iloc[0]
        history = st.session_state["price_history"].get(selected, [])

        # Today's OHLC candlestick (single bar; close tracks live LTP)
        fig_cs = _build_candlestick(row, selected)
        st.plotly_chart(fig_cs, use_container_width=True)

        # Session LTP sparkline
        ...
        if fig_sp:
            ...
            st.caption(
                f"Building session sparkline — {needed} more "
                f"refresh{'es' if needed != 1 else ''} needed (~{needed * refresh_s}s)."
            )

        # TFT quantile scatter for this stock
        if not pred_df.empty and selected in pred_df["Symbol"].values:
            ...
            st.info("No TFT forecast for this stock yet — inference consumer is pending model.")


# ── Tab 3: TFT Predictions ────────────────────────────────────────────────────
with tab_predictions:
    st.subheader("TFT Predictions — All Stocks")
    if pred_df.empty:
        st.info(
            "No predictions available.  \n"
            "The inference consumer needs a trained model checkpoint (`models/tft_v1.pth`).  \n"
            "Once it is running, `PRED:<SYMBOL>:quantiles` keys will populate here automatically."
        )
    else:
        st.caption(
            "5-quantile forecast at t+1 (next completed 1-min bar). "
            "Values are **log returns** — positive = expected gain, negative = expected loss. "
            "P50 is the median; P10/P90 are the 10th/90th percentile extremes."
        )
        ...
        try:
            ...
            ).map(_pred_color, subset=["P10", "P30", "P50", "P70", "P90"])
        except Exception:
...
# ── Sidebar stock detail (filled now that data & session_state are ready) ──────
selected = st.session_state.get("selected_stock")
if selected and selected in symbols:
    row    = df[df["Symbol"] == selected].iloc[0]
    sector = SECTOR_MAP.get(selected, "Other")

    with _stock_detail_slot.container():
        ...
        # Explicit stock switcher (fallback for users who prefer dropdown)
        new_sel = st.selectbox(
            "Switch stock",
            options=sorted(symbols),
            index=sorted(symbols).index(selected),
            key="sidebar_selector",
        )
        if new_sel != selected:
            st.session_state["selected_stock"] = new_sel
            st.rerun()

        # TFT quantile mini bar chart in sidebar
        if not pred_df.empty and selected in pred_df["Symbol"].values:
            pred_row = pred_df[pred_df["Symbol"] == selected].iloc[0]
            ...
            st.plotly_chart(fig_mini, use_container_width=True, key="sidebar_pred_bar")
